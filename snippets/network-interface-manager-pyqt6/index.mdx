---
title: Network Interface Manager (PyQt6)
description:
...
# Network Interface Manager (PyQt6)
...
- **Uses `nmcli` or `ip`**: Utilizes `nmcli` if available for managing
  interfaces, otherwise falls back to the `ip` command.
...
- **Troubleshooting**: Quickly enable or disable interfaces for troubleshooting
  network issues.
- **System Administration**: A useful tool for system administrators to monitor
...
- [**Python**](https://www.python.org): The programming language used for the
  application.
- [**PyQt6**](https://pypi.org/project/PyQt6/): A set of Python bindings for the
  Qt6 application framework, used for building the GUI.
...
Make sure Python 3 is installed. This can be checked by running
