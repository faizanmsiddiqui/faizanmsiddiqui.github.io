...
| Show command history  | `history`        | `Get-History` or `Get-Content (Get-PSReadLineOption).HistorySavePath` | `Get-History` shows current session history while `Get-Content (Get-PSReadLineOption).HistorySavePath` shows the saved history of all sessions. |
