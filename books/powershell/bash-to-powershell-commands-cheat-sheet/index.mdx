...
| Show command history  | `history`        | `Get-History` or `Get-Content (Get-PSReadlineOption).HistorySavePath` | `Get-History` shows current session history while `Get-Content (Get-PSReadlineOption).HistorySavePath` shows saved history of all sessions. |
